...
<div class="container-fluid py-4">
    <h2 class="mb-4">Gestion des Postulations</h2>
    <div class="row mb-3">
        <div class="col-md-6">
            <form method="get" action="index.php" class="d-flex align-items-center">
                <input type="hidden" name="controller" value="postulation">
                <input type="hidden" name="action" value="index">
                <div class="input-group">
                    <input type="text" class="form-control" name="search" placeholder="Rechercher une postulation par nom, prénom ou projet..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
                    <button class="btn btn-success" type="submit"><i class="fas fa-search"></i></button>
                </div>
                <a href="index.php?controller=postulation&action=index&sort=<?php echo (isset($_GET['sort']) && $_GET['sort'] === 'date_desc') ? 'date_asc' : 'date_desc'; ?>" class="btn btn-outline-primary ms-2">
                    <i class="fas fa-sort-amount-down-alt"></i> Trier par date
                </a>
            </form>
        </div>
    </div>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php if (empty($postulations)): ?>
                            <tr>
                                <td colspan="6" class="text-center">Aucune postulation trouvée</td>
                            </tr>
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="index.php?controller=postulation&action=edit&id=<?php echo $postulation['id']; ?>" class="btn btn-sm btn-success" title="Modifier"><i class="fas fa-edit"></i></a>
                                    <a href="index.php?controller=postulation&action=delete&id=<?php echo $postulation['id']; ?>" class="btn btn-sm btn-danger" title="Supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette postulation ?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
    ...
    <div class="text-center mt-4">
        <a href="index.php?controller=postulation&action=create" class="btn btn-success btn-lg">
            <i class="fas fa-plus"></i> Ajouter une nouvelle postulation
        </a>
    </div>
</div>
