document.addEventListener('DOMContentLoaded', function() {
    ...
    async function getAllFeedbacks(page = 1, limit = 10, search = '') {
        ...
        try {
            const response = await fetch(`http://localhost/startup2/controllers/back-office/feedbacks.php?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}&type=${encodeURIComponent(selectElement.value)}`);
            if (!response.ok) {
    ...
    async function getFeedbackById(id) {
        try {
            const response = await fetch(`http://localhost/startup2/controllers/back-office/feedbacks.php?id=${id}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            return await response.json();
    ...
    async function createFeedback(feedback) {
        try {
            const response = await fetch('http://localhost/startup2/controllers/back-office/feedbacks.php', {
                method: 'POST',
                headers: {
    ...
    async function getAllUsers() {
        try {
            const response = await fetch('http://localhost/startup2/controllers/back-office/users.php');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            return await response.json();
    ...
    function renderFeedbacks(feedbacks) {
        ...
        feedbacks.forEach(feedback => {
            ...
            switch(feedback.type) {
                case 'complaint':
                    typeBadge = `<span class="badge badge-red">Complaint</span>`;
                    break;
                case 'suggestion':
                    typeBadge = `<span class="badge badge-blue">Suggestion</span>`;
                    break;
                case 'praise':
                    typeBadge = `<span class="badge badge-green">Praise</span>`;
                    break;
                case 'question':
                    typeBadge = `<span class="badge badge-orange">Question</span>`;
                    break;
                default:
                    typeBadge = `<span class="badge">${feedback.type}</span>`;
            }
...
async function loadStatistics() {
    try {
        const statsUrl = 'http://localhost/startup2/controllers/back-office/feedback_stats.php';
...
document.querySelector('[data-tab="stats"]')?.addEventListener('click', loadStatistics);
    ...
    //     try {
    //         const response = await fetch(`http://localhost/startup2/controllers/back-office/feedbacks.php?page=${currentPage}&limit=${itemsPerPage}&search=${encodeURIComponent(this.currentSearch)}&type=${encodeURIComponent(this.currentType)}`);
    //         if (!response.ok) {
    //             throw new Error(`HTTP error! status: ${response.status}`);
    //         }
    //         renderFeedbacks(data);
