document.addEventListener('DOMContentLoaded', function() {
    ...
    async function getAllResponses(page = 1, limit = 10, search = '', type = '', isPublic = '', feedbackId = '') {
        try {
            let url = `/startup2/controllers/back-office/responses.php?page=${page}&limit=${limit}`;
    ...
    async function getResponseById(id) {
        try {
            const response = await fetch(`http://localhost/startup2/controllers/back-office/responses.php?id=${id}`);
            if (!response.ok) {
    ...
    try {
        ...
        const response = await fetch('http://localhost/startup2/controllers/back-office/responses.php', {
            method: 'POST',
            headers: {
    ...
    function renderResponses(responses) {
        ...
        responses.forEach(response => {
            ...
            switch(response.response_type) {
                case 'solution':
                    typeBadge = `<span class="badge badge-green">Solution</span>`;
                    break;
                case 'information':
                    typeBadge = `<span class="badge badge-blue">Information</span>`;
                    break;
                case 'rejection':
                    typeBadge = `<span class="badge badge-red">Rejection</span>`;
                    break;
                case 'follow_up':
                    typeBadge = `<span class="badge badge-orange">Follow Up</span>`;
                    break;
                default:
            ...
            row.innerHTML = `
                ...
                <td class="action-btns">
                    <button class="btn btn-secondary btn-sm edit-btn" data-id="${response.id}">
    ...
    async function openEditModal(id) {
        try {
            ...
            document.getElementById('feedback-id').value = response.feedback_id;
            document.getElementById('user-id').value = response.user_id;
...
async function loadFeedbacksForDropdown() {
    try {
        const response = await fetch('/startup2/controllers/back-office/feedbacks.php?limit=1000');
...
async function loadUsersForDropdown() {
    try {
        const response = await fetch('/startup2/controllers/back-office/users.php');
