(function ($) {
    ...
    async function loadFeedbacks(page = 1, limit = 6, search = '') {
        try {
            ...
            const response = await fetch(`http://localhost/startup2/controllers/back-office/feedbacks.php?${params}`);
    ...
    const feedback = {
        ...
        const response = await fetch('http://localhost/startup2/controllers/front-office/post_feedback.php', {
            method: 'POST',
        ...
        if (response.status === 401) {
            window.location.href = "http://localhost/startup2/controllers/front-office/check_login.php";
            return;
        }
    ...
    async function checkLoginStatus() {
        try {
            const response = await fetch('http://localhost/startup2/controllers/front-office/check_login.php');
            ...
            if (data.loggedIn) {
                ...
                document.getElementById('feedback-btn').style.display = 'inline';
                ...
                document.getElementById('form-feedback-card').style.display = 'none';
                const element = document.getElementById('feedback-carousel-display');
                element.className = '';
